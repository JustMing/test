 ...
 * @author ASUS
 */
public class RevenueSearchServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet RevenueSearch</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet RevenueSearch at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        if(a != null){
            ...
            if(name.equals("employee")){
                List<History> hl = sdb.searchEmp(owner, searchName);
                session.setAttribute("hisData", hl);
                request.getRequestDispatcher("jsp/revenue.jsp").forward(request, response);
            }if(name.equals("catagories")){
                List<History> hl = sdb.searchCata(owner, searchName);
                session.setAttribute("hisData", hl);
                request.getRequestDispatcher("jsp/revenue.jsp").forward(request, response);
            }
        }else if(e != null){
