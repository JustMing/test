 ...
 * @author ASUS
 */
public class CataSearchServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CataSearchServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CataSearchServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        List<Catagories> search = new ArrayList<>();
        for (Catagories c : cl) {
            if (c.getName().toLowerCase().contains(name.toLowerCase().trim())) {
                search.add(c);
            }
        }
        session.setAttribute("cataData", search);
